...
namespace Unclassified.TxLib
{
	...
	public static class Tx
	{
		...
		/// </remarks>
		private static Dictionary<string, Dictionary<string, Dictionary<int, string>>> languages = new();
		...
		/// Access to this object is synchronised through rwlock.
		/// </remarks>
		private static readonly Dictionary<string, FileSystemWatcher> fileWatchers = new();

		private static readonly object reloadTimerLock = new();
		...
		/// Access to this object is synchronised through rwlock.
		/// </remarks>
		private static readonly Dictionary<string, DateTime> fileTimes = new();
		...
		/// </summary>
		public static TxMode GlobalMode { get; set; }
		...
		private static void LoadFromXmlFile(string fileName, Dictionary<string, Dictionary<string, Dictionary<int, string>>> dict)
		{
			...
			using (new WriteLock(rwlock))
			{
				if (UseFileSystemWatcher)
				{
					...
					if (!fileWatchers.ContainsKey(fileName))
					{
						...
						{
							InternalBufferSize = 4096,   // Minimum possible value
							NotifyFilter = NotifyFilters.LastWrite | NotifyFilters.Security | NotifyFilters.Size | NotifyFilters.FileName
						};
						fsw.Changed += OnFileChanged;
						fsw.Created += OnFileChanged;
						fsw.Renamed += OnFileChanged;
			...
			if (m.Success)
			{
				var ci = CultureInfo.GetCultureInfo(m.Groups[1].Value);
			...
			{
				var ci = CultureInfo.GetCultureInfo(xe.Attributes["name"].Value);
		...
		private static void OnFileChanged(object sender, FileSystemEventArgs args)
		{
			...
			lock (reloadTimerLock)
			{
				...
				reloadTimer = new Timer(OnReloadTimer, null, ReloadChangesDelay, Timeout.Infinite);
			}
		}

		private static void OnReloadTimer(object state)
		{
			lock (reloadTimerLock)
			{
		...
		private static void LoadFromXml(string culture, XmlElement xe, Dictionary<string, Dictionary<string, Dictionary<int, string>>> dict)
		{
			...
			using (new WriteLock(isGlobalDict ? rwlock : null))
			{
				...
				if (!dict.TryGetValue(culture, out var language))
				{
				...
				foreach (XmlNode textNode in xe.SelectNodes("text[@key]"))
				{
					...
					if (language.TryGetValue(key, out var textItem))
					{
		...
		public static void AddText(string culture, string key, int count, int modulo, string text)
		{
			...
			{
				throw new ArgumentOutOfRangeException(nameof(count), "The count value must be in the range of 0 to ushort.MaxValue.");
			}
			if (modulo != 0 && (modulo < 2 || modulo > 1000))
			{
				throw new ArgumentOutOfRangeException(nameof(modulo), "The modulo value must be in the range of 2 to 1000.");
			}
			if (count == -1 && modulo > 0)
			{
			...
			{
				// Get a reference to the specified language dictionary
				if (!languages.TryGetValue(culture, out var language))
				{
					language = new Dictionary<string, Dictionary<int, string>>();
		...
		public static void Clear()
		{
			...
			{
				// Dispose all created FileSystemWatcher instances for the previously loaded files
				foreach (var fsw in fileWatchers.Values)
				{
					fsw.Dispose();
				}
				fileWatchers.Clear();
		...
		public static void ReplaceSystemTexts(Dictionary<string, Dictionary<string, Dictionary<int, string>>> systemTexts)
		{
			...
			// Remove all Tx:* keys
			foreach (KeyValuePair<string, Dictionary<string, Dictionary<int, string>>> language in languages)
			{
				foreach (string key in language.Value.Keys.ToArray())
				{
			...
			foreach (string languageCode in systemTexts.Keys)
			{
		...
		/// <summary>
		/// Adds support for <see cref="Tx"/>.
		/// </summary>
		/// <param name="app">The application builder.</param>
		public static void UseTx(this IApplicationBuilder app)
		{
		...
		public static CultureInfo[] AvailableCultures
		{
			get
			{
				...
				var cis = new CultureInfo[names.Length];
		...
		public static void SetWebCulture(string httpAcceptLanguage, string configPreference = null)
		{
			if (string.IsNullOrEmpty(httpAcceptLanguage))
			{
				...
				{
					httpPreferredCultures = Array.Empty<string>();
				}
			}
			else
			{
				// Parse every item from the list
				var cpList = new List<CulturePriority>();
				if (!string.IsNullOrEmpty(configPreference))
				{
		...
		{
			if (decimals < 0) throw new ArgumentOutOfRangeException(nameof(decimals), "Decimals must be greater than or equal to 0.");
		...
		public static string Percent(int value)
		{
			...
			string text = ResolveData(GetText(SystemKeys.Percent, -1, false, true), SystemKeys.Percent, -1, data);
			if (text == null)
			{
				text = numStr + "%";
			}
		...
		public static string Time(DateTime time, TxTime details, bool returnFormat = false)
		{
			...
			switch (details & TxTime.AnyDate)
			{
				...
				case TxTime.YearQuarter:
					...
					data = new Dictionary<string, string>
					{
						...
						["quarter"] = ((time.Month - 1) / 3 + 1).ToString()
					};
			...
			{
				var fi = (DateTimeFormatInfo)CultureInfo.CurrentCulture.DateTimeFormat.Clone();
				...
				while (true)
				{
					...
					else
					{
						...
						if (closeBracePos == -1)
						{
							...
							result.Append(text[openBracePos..]);
							break;
						}
						...
						else if (braceLength > 1 && text[openBracePos + 1] == '=')
						{
							...
							{
								// Count value {=...#...} found
								subkey = text[(openBracePos + 2)..countPos];
								string countName = text[(countPos + 1)..closeBracePos];
								if (countName != "")
								{
									if (usedPlaceholderNames != null)
										usedPlaceholderNames.Add(countName);
									if (data != null && data.TryGetValue(countName, out string countValue))
									{
						...
						else
						{
							...
							if (data != null && data.TryGetValue(varName, out string varValue))
							{
					...
					if (nextBracePos == -1)
					{
						...
						result.Append(text[(openBracePos + 1)..]);
						break;
					}
					else if (nextBracePos > openBracePos + 1)
					{
						// Copy text between this and the next brace
						result.Append(text[(openBracePos + 1)..nextBracePos]);
					}
		...
		{
			if (culture == null)
				throw new ArgumentNullException(nameof(culture), "The culture name must not be null.");

			if (key == null)
				throw new ArgumentNullException(nameof(key), "The text key must not be null.");
		...
		private static void LogUnusedPlaceholderNames(string key, Dictionary<string, string> data, HashSet<string> usedPlaceholderNames)
		{
			...
			{
				var dataNames = new HashSet<string>(data.Keys);
			...
			lock (logLock)
			{
				...
				if (logWriter != null)
				{
					if (usedKeys != null && !string.IsNullOrEmpty(primaryCulture))
					{
						...
						try
						{
							var primaryKeys = new HashSet<string>(languages[primaryCulture].Keys);
